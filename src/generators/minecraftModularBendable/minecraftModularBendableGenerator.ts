...
const script: ScriptDef = (generator: Generator) => {
  ...
  generator.defineBooleanInput("Show Foreground", true);
  generator.defineBooleanInput("Show Texture", true);
  /* generator.defineBooleanInput("Show Labels", true); */

  // Get user variable values

  const isAlexModel =
    generator.getSelectInputValue("Skin Model Type") === "Alex";
  ...
  const showForeground = generator.getBooleanInputValue("Show Foreground");
  const showTexture = generator.getBooleanInputValue("Show Texture");
  ...
  function drawHead([ox, oy]: Position) {
    ...
    minecraftGenerator.drawCuboid("Skin", char.base.head, [ox, oy], scale, {
      orientation: "South",
    });
    if (showHeadOverlay) {
  ...
  function drawLeftLeg([ox, oy]: Position) {
    const scale: Dimensions = [32, 96, 32];
    minecraftGenerator.drawCuboid("Skin", char.base.leftLeg, [ox, oy], scale, {
      center: "Right",
    });
  ...
  function drawPage1() {
    ...
    if (showTexture) {
      ...
      generator.drawTexture("Skin", [28, 24, 4, 4], [465, 457, 32, 32]); // Right Front Circle
      generator.drawTexture("Skin", [16, 24, 4, 4], [433, 457, 32, 32]); // Left Front Circle
    }
    if (showForeground) {
