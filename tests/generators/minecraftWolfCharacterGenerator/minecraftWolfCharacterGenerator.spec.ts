...
test("minecraft wolf character generator matches the default screenshot", async ({ page }) => {
  await page.goto("/generator/minecraft-wolf-character");
  ...
  for (let index = 0; index < 1; index += 1) {
    ...
    await expect(outputPage).toHaveScreenshot(
      "minecraft-wolf-character-default-page-" + (index + 1) + ".png"
    );
  }
});
