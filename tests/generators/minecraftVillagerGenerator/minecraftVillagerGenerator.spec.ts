...
test("minecraft villager generator matches the default screenshot", async ({ page }) => {
  await page.goto("/generator/minecraft-villager");
  ...
  for (let index = 0; index < 1; index += 1) {
    ...
    await expect(outputPage).toHaveScreenshot(
      "minecraft-villager-default-page-" + (index + 1) + ".png"
    );
  }
});
